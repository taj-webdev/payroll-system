...
/* ===============================
   AMBIL DATA GAJI (KUNCI KE KARYAWAN)
================================ */
$stmt = $conn->prepare("
    SELECT gd.*, 
    ...
    WHERE gd.id = ? AND gd.karyawan_id = ?
    LIMIT 1
");
$stmt->execute([$id, $karyawan_id]);
$data = $stmt->fetch();

if (!$data) {
    die('Data gaji tidak ditemukan atau bukan milik Anda');
}
