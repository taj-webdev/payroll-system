...
if ($search !== '') {
    $where[]  = "(p.nama LIKE ? OR k.nama_lengkap LIKE ? OR d.nama_divisi LIKE ?)";
    $params[] = "%$search%";
    $params[] = "%$search%";
    $params[] = "%$search%";
}
...
<div class="flex items-center justify-between mb-6">
    <h1 class="text-2xl font-bold flex items-center gap-2">
        <i data-lucide="minus-square" class="w-7 h-7 text-red-400"></i>
        Data Potongan
    </h1>

    <a href="potongan_add.php"
...
<form method="GET" class="glass p-4 mb-4">
    <div class="relative max-w-md">
        ...
        <input type="text" name="search"
               ...
               placeholder="Cari nama potongan / karyawan / divisi"
    ...
    <?php foreach($rows as $i => $r): ?>
        <tr class="border-b border-white/10 hover:bg-white/5">
            ...
            <td class="p-3 flex gap-2 justify-center">
                ...
                <a href="potongan_edit.php?id=<?= $r['id'] ?>"
...
function confirmDelete(id){
    Swal.fire({
        title: 'Hapus Data Potongan?',
        text: 'Data potongan akan dihapus',
        icon: 'warning',
        ...
        if(r.isConfirmed){
            window.location = 'potongan_delete.php?id=' + id;
        }
    })
}
...
Swal.fire({
    ...
    <?= json_encode(match($_GET['success']){
        'add'    => 'Berhasil Menambahkan Data Potongan',
        'edit'   => 'Berhasil Mengubah Data Potongan',
        'delete' => 'Berhasil Menghapus Data Potongan',
        default  => ''
    }) ?>
});
<?php endif; ?>
